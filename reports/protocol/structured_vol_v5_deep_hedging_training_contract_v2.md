# V5 Deep-Hedging Training Contract v2
...
Supersedes for future training authorization: `reports/protocol/structured_vol_v5_deep_hedging_training_contract_v1.md`
Reason: `Task-197 contract defects` (CUDA runtime not explicitly CUDA-only, GRU SiLU gate misstatement, NSDE output to price mapping not executable, synthetic premium not frozen, unit multiplier ambiguity)
...
- Deterministic price-path transformation (prospectively frozen, executable, traced from existing development-side methodology):
  ...
  1. NSDE forward: `increments = model.forward(context, noise)` where `noise` is standard normal `[batch, horizon=63, brownian_dim=2]` seeded per generator RNG schedule, `increments` is `[batch, horizon]` daily log-return increments `dx_t` (log-return per day).

  2. Cumulative log-return: `X_t = cumsum_{i=0}^{t} dx_i` with `X_{-1}=0` and `X_t` for `t=0..62` (inclusive). `X_t` is cumulative log-return level at close `t`.

  3. Tradable price path: `S_t = S_0 * exp(X_t)` where `S_0 = 100.0` (normalized initial reference level, prospectively frozen, development-only). `S_0` is fixed at `100.0` for all synthetic price paths (price scale is normalized; absolute level does not affect moneyness because `K = S_0 / m`). Alternative `x0=0` / normalized price scale in contract v1 is corrected to this exact `S_t = S_ref * exp(cumsum dx)` transformation.

  4. Properties: positivity guarantee via `exp()` (no clipping or floor required beyond exp; `S_t >0` always). No inverse normalization beyond `exp(cumsum)` (NSDE already operates on unnormalized log-return increments per `structured_vol_sde.py`; no per-dimension z-score inverse is applied to `S_t`). Dtype: synthetic price `S_t` is `float64` during cumsum/exp then cast to `float32` for hedger training; device is `cuda:0` (CUDA-only per Section 3.5) with deterministic `torch.Generator.manual_seed(synthetic_seed)` per generator.
...
- Synthetic option premium and unit/notional contract (load-bearing, frozen prospectively): synthetic training P&L requires `P0` premium per harness v3 Section 5.2 P&L formula. No frozen rule for synthetic premium existed in source. This repair freezes one reproducible synthetic premium contract entirely from synthetic episode state and already-frozen local assumptions, with no chronological final-test prices and no external option data.
  ...
  - Initial synthetic option premium `P0`: `INCLUDED` (same as evaluation premium convention). For each synthetic episode (with `S_0=100`, `K=S_0/m` where `m=S_0/K` is sampled moneyness in `[0.90,1.10]`, `T_0 = T_epi/252.0` at inception where `T_epi` is maturity itself, call/put type per 50/50), premium is Black-Scholes price using the same benchmark machinery bound in harness v3 Section 6, but with a fixed synthetic volatility input.

  - Pricing model/source used for synthetic `P0`: Black-Scholes benchmark formulas with `r=0.0` (same as harness v3 `r=0` continuously compounded) and `q=0.0` (continuous dividend, limitation stated, same as harness v3 `q=0`), `T_0 = T_epi/252.0` (maturity `5..30` sessions over 252), `S_0=100`, `K` as above, `vol = sigma_synth = 0.20` (20% fixed benchmark volatility, prospectively frozen, development-only). `sigma_synth=0.20` is a deterministic synthetic premium volatility, not a market IV; it is frozen before execution and not learned from final-test data.
  ...
  - Call/put treatment: call price `C = S_0*N(d1) - K*N(d2)`, put price `P = K*N(-d2) - S_0*N(-d1)` with `d1=(ln(S_0/K)+0.5*sigma_synth^2*T_0)/(sigma_synth*sqrt(T_0))`, `d2=d1 - sigma_synth*sqrt(T_0)`, `N(.)` normal CDF, `r=0` discount factor 1 (same as harness v3 call/put price formulas for T>0). At `T=0`, `C=max(S_0-K,0)`, `P=max(K-S_0,0)` (same as harness v3 T=0 handling).
  ...
  - P&L consistency: synthetic episode hedging P&L for training uses the same P&L formula as evaluation Section 5.2: `P&L = P0 + sum delta_{t-1}*(S_t - S_{t-1}) - Payoff_T - sum costs` where `P0` is the synthetic Black-Scholes premium above, `Payoff_T` is intrinsic at synthetic `S_T` (same European payoff call `max(S_T-K,0)` put `max(K-S_T,0)`), costs are synthetic `c*|delta_t - delta_{t-1}|*S_t` including unwind Yes at synthetic `S_T`.

  - Unit/notional convention (frozen as one convention across all legs): one option unit with multiplier `1` (not `100`). Premium `P0`, payoff `Payoff_T`, hedge position `delta_t` (shares of underlying per 1 unit spot notional), underlying P&L `delta_{t-1}*(S_t - S_{t-1})`, transaction cost `c*|delta_t - delta_{t-1}|*S_t` are all per `1` unit spot notional with multiplier `1`. Research normalization is one option unit with multiplier `1`, not one listed SPY contract with multiplier `100`. No mixed units; evaluation H3 convention is same (one unit, multiplier 1). This preserves H3 evaluation convention.

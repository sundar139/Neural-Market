# V5 Final-Test Single-Access Harness v3
...
Supersedes for future authorization: `reports/protocol/structured_vol_v5_final_test_single_access_harness_v2.md`
Reason: `Task-194 CVaR-estimator residual promoted to load-bearing reproducibility requirement` (fractional-tail empirical Expected Shortfall not explicitly pinned in v1/v2/SAP beyond CVaR_0.95 label)

This task is a one-issue prospective contract repair only. It preserves harness v1, harness v2, Amendment 097, and Amendment 098 unchanged. It grants no final-test access entitlement. No final-test row access, no training, no hedger execution, no generator execution, no Gate, no model inference, no bootstrap execution, no validation, no external validation, no network, no push.
...
Final-test state at repair: `SEALED`, access count `0`, entitlement `NONE`, authorization `NOT GRANTED`, harness v1 `REPAIR_REQUIRED_PRESERVED`, harness v2 `VALIDATED_EXCEPT_CVAR_ESTIMATOR_PRECISION`, deep hedging `NOT READY` and training contract `NOT FROZEN`.

## 2. Preserved harness-v2 values (not redesigned except CVaR)

The following remain exactly as frozen in v2 (except CVaR estimator now added) (not redesigned per bounded-repair scope):
...
## 4B. Exact empirical 95% CVaR estimator (REPAIRED — fractional-tail Expected Shortfall)

This section prospectively freezes the exact finite-sample empirical CVaR estimator that was not explicitly pinned in v1/SAP/harness v2 beyond "CVaR_0.95 of hedging loss" and therefore required bounded repair before hedger training or final-test access. Original protocol defines L = -P&L and Delta_CVaR formula with alpha=0.95 but does not specify quantile/tail convention, boundary fractional weighting, or tie handling. No stronger source-native estimator exists in committed source (research_protocol_v1.md, Amendment 020, SAP v1, harness v1/v2). This repair freezes one deterministic estimator now.

### 4B.1 Estimator definition

For finite loss vector x = [x_1,...,x_N] with all invalid/missing episodes already removed under the frozen missingness contract (Section 5.5), so N is the count of valid hedging losses for that policy/cost level/replicate.
...
- if tail_mass <= 0: INSUFFICIENT_SAMPLE (requires N >= 1 with alpha<1; with alpha=0.95 this only occurs at N=0; already handled by N>=40 precondition).

- if k == 0 and f > 0 (i.e., 0 < tail_mass < 1, which occurs at N < 20): CVaR = x_(N) (the single largest loss). With the frozen minimum N=40, this branch is not reached for valid samples, but it is deterministically defined.
...
- N >= 40 is the frozen minimum (Section 4.1 N >= 2*L, Section 4B.3 reconciled to 40). At alpha=0.95 and N=40, tail_mass = 2.0, k=2, f=0, CVaR is mean of 2 largest losses.

- No other case requires special handling.

### 4B.2 Sort, ties, NaN, weights, interpolation, VaR

- Sort: ascending stable numeric order (Python sorted / NumPy sort with stable kind, deterministic).

- Ties: retained as distinct episode observations; equal losses are ordered by stable sort and counted as separate empirical mass; fractional boundary weighting is by ordered empirical mass, not by unique value. No collapsing of ties.

- NaN/inf: excluded only under already-frozen episode missingness rules BEFORE CVaR (Section 5.5 and Section 4.1). CVaR never sees NaN/inf as input; if all N are removed, N=0 leads to INSUFFICIENT_SAMPLE. No NaN is passed to sort.

- Weights: equal episode weight 1/N. No observation weighting.

- Interpolation: NONE beyond the explicit fractional boundary mass above. No np.quantile, no linear interpolation of VaR, no scipy helper. VaR is report-only boundary value not separately used to redefine membership. The only fractional weighting is the boundary observation weight f as defined.

- VaR role: VaR_0.95 = x_(N - k) when f==0, or x_(N-k) as boundary when f>0 (the threshold observation). VaR is reported as boundary value for diagnostics only, not used to define a separate strict >VaR vs >=VaR membership; membership is exactly the fractional tail mass rule above.

- Deterministic: estimator is deterministic for all N given the same valid loss vector and sort. No randomness, no alternative CVaR estimator.

### 4B.3 Sample-size and failure reconciliation

- Minimum N: 40 (preserved from CBB precondition N >= 2*L = 40). Reconciliation: at alpha=0.95 and N=40, tail_mass=2.0, so two effective tail observations are represented; this is sufficient for the fractional-tail estimator and no stricter minimum is required by committed source. Preserve missing/nonfinite rate rules (no imputation, >0.1% nonfinite triggers failure), no silent denominator change, no alternative fallback CVaR.

- For a bootstrap replicate with valid resampled N (resampled episode set I_b filtered to valid episodes per missingness contract): CVaR is computed deterministically via Section 4B.1 on the resampled loss vector of length N_valid (which equals N if no missing in replicate, or smaller if missingness rules removed some). If N_valid < 40, the replicate is still evaluated with the same estimator but the overall CBB precondition for the point estimate (N>=40) already ensures primary sample is sufficient; a bootstrap replicate with N_valid < 40 is still valid but contributes tail_mass = 0.05*N_valid (e.g., N_valid=38 tail_mass=1.9). No INSUFFICIENT_SAMPLE per replicate except when tail_mass<=0 (N_valid=0). If Deep or BS CVaR becomes nonfinite for a replicate, that replicate becomes invalid according to existing B_valid semantics (Section 4.4, B_valid==B required).

- Existing bootstrap validity requirement B_valid == 10000 (all 10000 replicates finite) remains unchanged.
...
1. `Git HEAD` matches authorizing HEAD that includes this harness v3 and its audit — mismatch abort.
...
4. Harness v3 identity: `reports/protocol/structured_vol_v5_final_test_single_access_harness_v3.md` canonical SHA and Git blob as frozen in Amendment 099 — mismatch abort.
...
- Task: `NM-R4-V5-FINAL-TEST-SINGLE-ACCESS-HARNESS-CVAR-ESTIMATOR-REPAIR-195` — `R4`.
- No source code change, no harness v1/v2 edit, no hedging execution, no deferred values, no network, no push.
- This harness v3 plus Amendment 099 are `R4` protocol artifacts; they add the CVaR-estimator precision repair pending re-audit. Deep hedging training contract remains `NOT YET FROZEN`, policies `NOT YET AVAILABLE`.
...
- Harness v2 canonical `7a28cb149e58804919babdcb70d19e6c960d97634776ea592bb67acb67ec7ec6` Git blob `676c5932ee4ee38db56d37b6c80a8943b0025237` — preserved, not edited.
...
- Amendment 098 canonical `487a666f10930e730e4e9c5dd03d79eaa6744968441543941f29e9d43becadb7` Git blob `40666ace19bc2b4878780799d1fe3d22dab4b8c3` — preserved.
...
*This harness is append-only. Any change requires new governed amendment, not silent edit. Next governed action is the independent CVaR-estimator audit task NM-R4-V5-FINAL-TEST-SINGLE-ACCESS-HARNESS-CVAR-ESTIMATOR-AUDIT-196.*
